function [ncp] = NCPLDA1(MiFunSim,DiPheSim,mi2diNetwork,A)
	...
	for i = 1:MnRow
		for j = 1:DnCol
            ncp(i,j) = (A*ncp_m(i,j) + (1-A)*ncp_d(i,j));
			%ncp(i,j) = (A*ncp_m(i,j) + (1-A)*ncp_d(i,j))/0.5*(norm(MiFunSim(i,:))+norm(DiPheSim(:,j)));
